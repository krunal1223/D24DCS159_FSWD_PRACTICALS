...
import { OrbitControls, ContactShadows, Text, useGLTF } from "@react-three/drei"
...
export function FurniturePreview({ specs, modelUrl }: FurniturePreviewProps) {
  return (
    <div className="w-full h-full bg-gradient-to-br from-slate-100 to-slate-200 relative">
      <Canvas camera={{ position: [5, 5, 5], fov: 50 }} shadows>
        <Suspense fallback={<LoadingFallback />}>
          <Scene specs={specs} modelUrl={modelUrl} />
        </Suspense>
      </Canvas>
...
function Scene({ specs, modelUrl }: { specs: FurnitureSpec; modelUrl?: string }) {
  return (
    <>
      {/* Lighting */}
      ...
      {/* Furniture Model */}
      {modelUrl ? (
        <CustomModel modelUrl={modelUrl} specs={specs} />
      ) : specs.category ? (
...
function CustomModel({ modelUrl, specs }: { modelUrl: string; specs: FurnitureSpec }) {
  const { scene } = useGLTF(modelUrl)
  const meshRef = useRef<THREE.Group>(null)

  // Scale the model based on dimensions
  const scale = Math.min(specs.dimensions.width / 50, specs.dimensions.height / 50, specs.dimensions.depth / 50)
